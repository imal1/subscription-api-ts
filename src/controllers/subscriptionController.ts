...
export class SubscriptionController {
  ...
  index = (req: Request, res: Response): void => {
    ...
    const isApiRequest =
      acceptHeader.includes("application/json") || req.query.format === "json";
  ...
  getSubscriptionFile = async (req: Request, res: Response): Promise<void> => {
    try {
      const content = await this.subscriptionService.getFileContent(
        "subscription.txt"
      );
  ...
  getClashFile = async (req: Request, res: Response): Promise<void> => {
    try {
      const content = await this.subscriptionService.getFileContent(
        config.clashFilename
      );

      res.setHeader("Content-Type", "text/yaml; charset=utf-8");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${config.clashFilename}"`
      );
      res.send(content);
    } catch (error: any) {
    ...
    try {
      ...
      for (const testNode of testNodes) {
        ...
        try {
          ...
          logger.info(
            `${protocol} 转换成功: ${proxyCount} 个节点，类型: ${outputType}`
          );
        } catch (error: any) {
          results.tests.push({
            protocol,
      ...
      try {
        ...
        logger.info(
          `组合转换结果: ${allProxyCount} 个节点，类型分布: ${JSON.stringify(
            allOutputTypes
          )}`
        );
      } catch (error: any) {
   ...
   * 转换内容为 Clash 配置
   */
  convertContent = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const response: ApiResponse = {
        ...
        data: {
          ...
          configLength: clashConfig.length,
        },
        message: "转换成功",
        timestamp: new Date().toISOString(),
      };
