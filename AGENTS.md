...
- **多节点层级**：主节点负责统一拉取所有节点源并生成 `raw.txt`、`subscription.txt`、`clash.yaml`；子节点只运行 Agent 和代理内核，提供 `/api/urls` 节点源，不在子节点生成订阅文件、Clash 配置或 mihomo 输出。
- **远端 Agent 访问**：常规状态、健康检查、节点源收集只通过公网 `http://<host>:<agentPort>` + HMAC；不要在正常路径用 SSH fallback 调远端 `127.0.0.1:3001`，否则会掩盖安全组/防火墙未放行的问题。SSH 只用于部署和诊断。
- **远端端口**：部署/管理默认需要 SSH `22/tcp` 与 Agent `3001/tcp` 入站放行；实际代理还需要放行内核配置中的监听端口（例如 233boy/sing-box 生成的 VLESS Reality 端口）。主站本机只需能出站访问这些远端端口。
- **233boy 内核布局**：Agent 默认兼容 233boy/sing-box、xray、v2ray 的常见布局，扫描 `/etc/<kernel>/config.json`、`/etc/<kernel>/conf/*.json` 以及 `/usr/local/etc/<kernel>/...`。sing-box 测试节点实际使用 `/etc/sing-box/config.json` + `/etc/sing-box/conf/*.json`。
...
cd frontend && bunx tsc --noEmit  # 前端/Next 类型检查
cd agent && bun test              # Agent 单元测试
...
> 注意：仓库根目录的旧 `tsconfig.json` 来自迁移前结构，`npx tsc --noEmit` 在根目录会因为找不到 `src/**/*` 而失败。CI 或本地检查应使用 `frontend/tsconfig.json`（例如 `cd frontend && bunx tsc --noEmit`），或者显式调整 workflow。
...
`.Codex/memory/` 目录包含持久化的项目记忆文件。每次完成以下操作后，必须更新对应的记忆文件：

| 操作 | 更新文件 | 更新方式 |
|------|---------|---------|
| 修复 bug | `.Codex/memory/bug-fixes.md` | 在文件顶部追加新条目（日期、症状、原因、修复） |
| 架构决策 / 技术选型变更 | `.Codex/memory/project-architecture.md` | 追加新的决策条目 |
| CI/CD workflow 变更 | `.Codex/memory/ci-cd-pipeline.md` | 更新对应章节 |
| 部署流程变更 | `.Codex/memory/deployment-flow.md` | 更新对应章节 |
| 配置约定变更 | `.Codex/memory/config-patterns.md` | 更新对应配置项 |
| 代码风格 / lint 规则变更 | `.Codex/memory/coding-conventions.md` | 更新对应约定 |
...
索引文件 `.Codex/memory/MEMORY.md` 维护所有记忆文件的链接列表。新增记忆文件时同步更新索引。
